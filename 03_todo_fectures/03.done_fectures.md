...
### done-20260617 付費版（Pro）買斷 IAP — 已上架 v1.3.20260615 (build 14)
- 一次性 non-consumable `app.rexcode.sunnywalker.pro.lifetime`、**US$1.99**、家庭共享；舊用戶以 `AppTransaction.originalAppVersion` grandfather 自動免費升 Pro。
- 解除 3 項可達上限：鬧鐘 6→∞、自定鈴聲 5→∞、單則鈴聲 5s→30s（單一來源 `FeatureLimits`）。
- 購買入口在設定頁家長驗證之後，含 Restore Purchases。
- 原始規劃草案（買斷 vs 訂閱、定價分析，2026-06-08）保留於 `03_todo_fectures_done.md`；送審細節見 `release_note/apple_store.md`。

### done-20260607 新增鬧鐘頁
- 鈴聲分兩列 + 口令提示優化（詳見 `03_todo_fectures_done.md`）。
...
### 已上架的 GUI 親合度／UX 調整（v1.0.20260612 → 1.3.20260615，補登）
> 來源：`release_note/apple_store.md`、官網 `release-notes`。吉卜力動畫大改之外，這幾版實際已做的「順手度」優化，先記在這裡備忘。

**v1.0.20260612 (build 7) — 錄音流程**
- 錄音／命名／試聽**整合成單頁**，一次完成；新增「重新錄音」鈕。
- 「辨識錄音內容」自動命名（中文 8、英文 16 字上限）。
- 補齊英文版缺漏字串、修中英混排（避免審核退件）。

**v1.1.20260612 (build 9) — 溫和提醒**
- per-alarm「溫和提醒模式」（Time-Sensitive 通知，響 30s 自動停）。
- 通知 body 直接顯示鬧鐘 label；點通知回主畫面、不再強開鬧鐘畫面。

**v1.2.20260613 (build 10) — 設定頁與試聽**
- 溫和提醒新增 per-alarm「切段響滿 30 秒」開關（預設關＝只響一下）。
- 「錄音管理」回到設定頁第一項（與新增鬧鐘頁同一入口）。
- 自訂鈴聲試聽：**點一下播放／暫停、長按停止**；移除設定頁 DEBUG probe。

**v1.3.20260615 (build 14) — 匯入、修音、Pro 頁**
- 「選鈴聲」與「錄音管理」新增**「匯入音檔」**鈕（從檔案 App 選 mp3/wav…）＋ DRM 友善錯誤 alert。
- 修：內建鈴聲在「溫和提醒（通知）模式」終於會響（先前只「咚」一聲系統音）。
- Pro 購買頁排版一致化：benefit 字級對齊設定列（16pt）、單則鈴聲文案統一為「免費版 5 秒，Pro 最長 30 秒」。
- 移除「家長錄音長度」這條**到不了的 benefit**，購買頁只剩 3 項可達上限。
- 家長暫時解鎖 UX：通過家長驗證後自動開啟解鎖視窗（預設 5 分鐘）＋新增「立即上鎖」。
- 價格一律走 `product.displayPrice`，畫面不寫死、不標 USD。
...
> 🛑 **此為原始草案、僅存查。最終定價為 US$1.99 base（台灣約 NT$60），已隨 1.3.20260615 (build 14) 送審。** 下文 NT$90／NT$120／訂閱等定價方向皆未採用；以 `release_note/apple_store.md` 與 `03.todo_Premium_Features_fable_260616_page_field.md` 為準。
