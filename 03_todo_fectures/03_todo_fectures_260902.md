...
### done-20260917_2340 報時「倒數」選項（＋中／英文）
- 間隔本來就可選 1／2／3／5／10／15／20／30 分（選 10 分＝每 10 分報一次）。
- 區間報時多一個「倒數報時」開關：時刻不變，改念離「迄」還剩多久——起 7:00 迄 7:30 每 10 分 →
  「剩三十分鐘、剩二十分鐘、剩十分鐘」（≥60 分念「剩一小時三十分鐘」）；迄本身仍不報。
  卡片預覽列出孩子會聽到的話，試聽念第一句，首頁卡片顯示「每 10 分 · 倒數」。
- 中／英文：介面字串雙語；語音與通知橫幅跟 App 語言走（英文 "Twenty minutes left."）。
  切換 App 語言會當下重排報時鬧鐘並用新語言重新合成（一般報時以前切語言後還念舊語言，一併修掉）。
- +6 單元測試（剩餘分鐘、中英句子、語言標籤），共 119 全綠。

### done-20260917_2350 群組列的報時／待辦圖示：點了要說明啟用了什麼
- 設定 › 群組：點鈴鐺／氣球＝切換，說明顯示在群組段**最下面**的固定高度分頁（命名／報時／待辦提醒），
  點圖示自動切到對應那頁並標出「群組 B 已開啟」——不在圖示下面冒字，畫面不會跳（Rex 260917 回饋）。
  長按＝只看說明不切換。多一行小字提醒「這一組原有的鬧鐘要打開再儲存一次才會套用」。中英文、VoiceOver 都有。
